...
import { FIRST_TAG_REG, IMAGE_URL_REG, LINK_REG, MARKDOWN_URL_REG, MD_LINK_REG, MEMO_LINK_REG, TAG_REG, WIKI_IMAGE_URL_REG } from '../helpers/consts';
...
import { showMemoInDailyNotes } from '../obUtils/Memos/obShowMemo';
...
import { CommentOnMemos, CommentsInOriginalNotes, DefaultEditorLocation, ShowCommentOnMemos, ShowTaskLabel, UseButtonToShowEditor } from '../memosView';
...
const Memo: React.FC<Props> = (props: Props) => {
        ...
        try {
            if (commentMemoId) {
                ...
                if (!CommentsInOriginalNotes) {
                    content = content + ' comment: [[' + moment(propsMemo.id.slice(0, 8)).format(dailyFormat) + '#^' + randomId + ']]';
                }
                ...
                setCommentMemos([...commentMemosRef.current, newMemo].sort((a, b) => utils.getTimeStampByDate(b.createdAt) - utils.getTimeStampByDate(a.createdAt)));
                // setCommentMemos(
    ...
    const handleMarkMemoClick = () => {
        if (UseButtonToShowEditor && DefaultEditorLocation === 'Bottom') {
            const elem = document.querySelector("div[data-type='memos_view'] .view-content .memo-show-editor-button") as HTMLElement;
        ...
        >
            <div className="memo-top-wrapper">
                <div className="memo-top-left-wrapper">
                    ...
                    <div className={`memo-type-img ${(propsMemo.memoType === 'TASK-TODO' || propsMemo.memoType === 'TASK-DONE') && ShowTaskLabel ? '' : 'hidden'}`}>{handleMemoTypeShow() ?? ''}</div>
                </div>
            ...
            <div className="memo-content-text" onClick={(e) => handleMemoContentClick(e, propsMemo)} onDoubleClick={handleMemoDoubleClick} dangerouslySetInnerHTML={{ __html: formatMemoContent(propsMemo.content, propsMemo.id) }}></div>
