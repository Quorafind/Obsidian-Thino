                    ...
                    if (dataArr[i].hasId !== undefined) {
                        ...
                        if (commentMemos.length > 0) {
                            commentMemos.map((cm) => {
                                ...
                                outputText = outputText + indent + (ShowDate ? '    ' + memoType + '[[' + moment(cm.createdAt).format(dailyNotesformat) + ']] ' : '    ' + memoType) + moment(cm.createdAt).format('HH:mm') + ' ' + cm.content.replace(/comment:(.*)$/g, '').replace(/^\d{14}/g, '') + '\n';
                            });
                        }
                    }
                }
